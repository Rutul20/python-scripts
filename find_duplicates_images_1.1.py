...
STEP 1 -- Put this file in the right place
    Copy "find_duplicates_images_1.1.py" directly into the folder that contains
...
STEP 4 -- Similar-image detection (automatic, no setup needed)
    To catch photos that just LOOK the same (resized, re-saved, lightly
    edited copies), not just exact duplicates, this script needs the
    Pillow library. You don't need to install anything yourself: if
    Pillow isn't already on your machine, the script installs it
    automatically the first time it needs it, uses it for that run,
    and then removes it again afterwards -- so it leaves nothing extra
    behind on your computer. This needs an internet connection the
    first time; if none is available, the script just skips the
    similar-image check and still finds exact duplicates fine.
    (If Pillow was already installed before you ran this script --
...
Two kinds of matches are found:
  ...
  2. Similar images -- photos that look the same to the eye but aren't
     ...
     requires the Pillow library. This script installs Pillow
     ...
     it just skips the similar-image step.
...
Optional command-line flags (advanced -- plain `python find_duplicates_images_1.1.py`
with no flags is all most people need):
  python find_duplicates_images_1.1.py                   # scan this script's folder
  python3 find_duplicates_images_1.1.py                  # same, on macOS/Linux
  python find_duplicates_images_1.1.py --recursive        # also scan subfolders
  python find_duplicates_images_1.1.py --dry-run          # only report, don't move anything
  python find_duplicates_images_1.1.py --no-similar       # skip similar-image detection
  python find_duplicates_images_1.1.py --threshold 8      # looser similarity match (default: 5)
  python find_duplicates_images_1.1.py --csv report.csv
  python find_duplicates_images_1.1.py --version          # print the script version
"""
...
def run():
    ...
    print(f"find_duplicates_images_1.1.py v{__version__}")
    ...
    similar_groups = []
    pillow_installed_by_us = False
    try:
        if not args.no_similar:
            available, pillow_installed_by_us = ensure_package("PIL", "Pillow")
            if not available:
                print("Skipping similar-image detection (use --no-similar to silence this message).", file=sys.stderr)
            else:
        ...
        if pillow_installed_by_us:
            print("\nCleaning up: removing the Pillow library this run installed temporarily...")
